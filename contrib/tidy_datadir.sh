#!/bin/bash
# Copyright (c) 2013 The Syndicate Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Syndicate database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Syndicate datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Syndicate datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Syndicate 0.7 datadir."
    ;;
  3)
    echo "Detected Syndicate pre-0.8 datadir."
    ;;
  4)
    echo "Detected Syndicate 0.8 datadir."
    ;;
esac
