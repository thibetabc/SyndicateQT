...
---------------------
A bug in the automint calculations was made apparent on mainnet when block times exceeded expectations, resulting in zSYNX mint transactions that were in an unconfirmed state to still be treated as if they had never been minted. This caused automint to effectively mint more than what was intended.

zSYNX Spending Fix
---------------------
The size of zSYNX spend transactions is knowingly larger than normal transactions, and while this was expected, a much stricter check against the scriptsig size is used for mainnet, causing the transactions to be rejected by the mempool, and thus not being packaged into any blocks.

zSYNX Transaction Recovery
---------------------
Due to the aforementioned issue with zSYNX spending, users may find that their attempted spends are now conflicted and zSYNX balances are not represented as expected. "Recovery" of these transactions can be done using the following methods:
...
- #292 `feadab4` Additional checks for double spending of zSynx serials. (presstab)

### Wallet
- #271 `5e9a086` [Wallet] Remove unused member wallet in UnlockContext inner class (Jon Spock)
- #279 `e734010` Add -backupzsynx startup flag. (presstab)
- #280 `fdc182d` [Wallet] Fix zSynx spending errors. (presstab)
- #282 `310f216` [Wallet] Count pending zSynx balance for automint. (presstab)
...
- #291 `cb314e6` [Qt] zSynx control quantity/amount fixes (rejectedpromise)
