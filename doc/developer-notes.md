...
Run with the -testnet option to run with "play syndicates" on the test network, if you
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_syndicate
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_syndicate --log_level=test_suite
$ valgrind -v --leak-check=full src/syndicated -printtoconsole
```
...
# A coverage report will now be accessible at `./test_syndicate.coverage/index.html`.
```
...
- SyndicateMiner : Generates syndicates (if wallet is enabled).
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:syndicate/syndicate.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `syndicate-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `syndicate-cli` and the GUI debug console use this table to determine how to
