...
Value stashedsend(const Array& params, bool fHelp)
{
    ...
    if(params[0].get_str() == "auto"){
        if(fMasterNode)
            return "StashedSend is not supported from masternodes";

        return "DoAutomaticDenominating " + (stashedSendPool.DoAutomaticDenominating() ? "successful" : ("failed: " + stashedSendPool.GetStatus()));
    }

    if(params[0].get_str() == "reset"){
        stashedSendPool.Reset();
        return "successfully reset stashedsend";
    }
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("state",        stashedSendPool.GetState()));
    obj.push_back(Pair("entries",      stashedSendPool.GetEntriesCount()));
    obj.push_back(Pair("entries_accepted",      stashedSendPool.GetCountEntriesAccepted()));
    return obj;
}
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  stop         - Stop masternode configured in syndicate.conf\n"
                ...
                "  vote-many    - Vote on a Syndicate initiative\n"
                "  vote         - Vote on a Syndicate initiative\n"
                );
    ...
    if (strCommand == "genkey")
    {
        ...
        return CSyndicateSecret(secret).ToString();
    }
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CMasternodeConfig::CMasternodeEntry mne, masternodeConfig.getEntries()) {
            ...
            if(!stashedSendSigner.SetKey(mne.getPrivKey(), errorMessage, keyMasternode, pubKeyMasternode)){
            ...
            if(!stashedSendSigner.SignMessage(strMessage, errorMessage, vchMasterNodeSignature, keyMasternode)){
                printf(" Error upon calling SignMessage for %s\n", mne.getAlias().c_str());
                failed++;
                continue;
            }

            if(!stashedSendSigner.VerifyMessage(pubKeyMasternode, vchMasterNodeSignature, strMessage, errorMessage)){
    ...
    if(strCommand == "vote")
    {
        ...
        if(!stashedSendSigner.SetKey(strMasterNodePrivKey, errorMessage, keyMasternode, pubKeyMasternode))
...
Value masternodelist(const Array& params, bool fHelp)
{
                ...
                && strMode != "pubkey" && strMode != "rank" && strMode != "status" && strMode != "addr" && strMode != "votes" && strMode != "lastpaid"))
    {
        throw runtime_error(
                ...
                "  votes          - Print all masternode votes for a Syndicate initiative (can be additionally filtered, partial match)\n"
