...
{
    return QCoreApplication::translate("syndicate-core", psz).toStdString();
}
...
static void handleRunawayException(std::exception *e)
{
    ...
    QMessageBox::critical(0, "Runaway exception", SyndicateGUI::tr("A fatal error occurred. Syndicate can no longer continue safely and will quit.") + QString("\n\n") + QString::fromStdString(strMiscWarning));
    exit(1);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(syndicate);
    Q_INIT_RESOURCE(syndicate_locale);
    ...
    // Load e.g. syndicate_de.qm (shortcut "de" needs to be defined in syndicate.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        app.installTranslator(&translatorBase);

    // Load e.g. syndicate_de_DE.qm (shortcut "de_DE" needs to be defined in syndicate.qrc)
    ...
    {
        MacDockIconHandler::instance()->setIcon(QIcon(":icons/syndicate_testnet"));
    }
#endif
    ...
    try
    {
        ...
        if(AppInit2(threadGroup))
        {
            ...
            // Shutdown the core and its threads, but don't exit Syndicate-Qt here
