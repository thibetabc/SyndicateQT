    ...
    model(0)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_STASHEDSEND_ROUNDS, 120);
...
{
    GUIUtil::setClipboard(SyndicateUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            CTxIn vin(outpt);
            int rounds = pwalletMain->GetInputStashedsendRounds(vin);
            if(coinControl->useStashedSend && rounds < nStashedsendRounds) {
                ...
                coinControl->useStashedSend = false;
            }
        }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            // DS Fee = overpay
            if(coinControl->useStashedSend && nChange > 0)
            {
                nPayFee += nChange;
                nChange = 0;
            }
    ...
    int nDisplayUnit = SyndicateUnits::SYNX;
    ...
    l2->setText(SyndicateUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(SyndicateUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(SyndicateUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? "~" : "") + QString::number(nBytes));                                    // Bytes
    l6->setText(sPriorityLabel);                                             // Priority
    l7->setText((fLowOutput ? (fDust ? tr("DUST") : tr("yes")) : tr("no"))); // Low Output / Dust
    l8->setText(SyndicateUnits::formatWithUnit(nDisplayUnit, nChange));        // Change

    // turn labels "red"
    l5->setStyleSheet((nBytes >= 10000) ? "color:red;" : "");               // Bytes >= 10000
    l6->setStyleSheet((dPriority <= 576000) ? "color:red;" : "");         // Priority < "medium"
    l7->setStyleSheet((fLowOutput) ? "color:red;" : "");                    // Low Output = "yes"
    l8->setStyleSheet((nChange > 0 && nChange < CENT) ? "color:red;" : ""); // Change < 0.01ION

    // tool tips
    l5->setToolTip(tr("This label turns red, if the transaction size is bigger than 10000 bytes.\n\n This means a fee of at least %1 per kb is required.\n\n Can vary +/- 1 Byte per input.").arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)));
    l6->setToolTip(tr("Transactions with higher priority get more likely into a block.\n\nThis label turns red, if the priority is smaller than \"medium\".\n\n This means a fee of at least %1 per kb is required.").arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)));
    l7->setToolTip(tr("This label turns red, if any recipient receives an amount smaller than %1.\n\n This means a fee of at least %2 is required. \n\n Amounts below 0.546 times the minimum relay fee are shown as DUST.").arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)).arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)));
    l8->setToolTip(tr("This label turns red, if the change is smaller than %1.\n\n This means a fee of at least %2 is required.").arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)).arg(SyndicateUnits::formatWithUnit(nDisplayUnit, CENT)));
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            {
                sAddress = QString::fromStdString(CSyndicateAddress(outputAddress).ToString());

                // if listMode or change => show syndicate address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, SyndicateUnits::format(nDisplayUnit, out.tx->vout[out.i].nValue));
            ...
            if(rounds >= 0) itemOutput->setText(COLUMN_STASHEDSEND_ROUNDS, strPad(QString::number(rounds), 15, " "));
            else itemOutput->setText(COLUMN_STASHEDSEND_ROUNDS, strPad(QString(tr("n/a")), 15, " "));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, SyndicateUnits::format(nDisplayUnit, nSum));
