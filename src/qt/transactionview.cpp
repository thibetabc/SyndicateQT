    ...
    transactionView(0)
{
    ...
    typeWidget->addItem(tr("Darksend Make Collateral Inputs"), TransactionFilterProxy::TYPE(TransactionRecord::StashedsendMakeCollaterals));
    typeWidget->addItem(tr("Darksend Create Denominations"), TransactionFilterProxy::TYPE(TransactionRecord::StashedsendCreateDenominations));
    typeWidget->addItem(tr("Darksend Denominate"), TransactionFilterProxy::TYPE(TransactionRecord::StashedsendDenominate));
    typeWidget->addItem(tr("Darksend Collateral Payment"), TransactionFilterProxy::TYPE(TransactionRecord::StashedsendCollateralPayment));
...
void TransactionView::changedAmount(const QString &amount)
{
    ...
    // Replace "," by "." so SyndicateUnits::parse will not fail for users entering "," as decimal separator
    QString newAmount = amount;
    newAmount.replace(QString(","), QString("."));

    if(SyndicateUnits::parse(model->getOptionsModel()->getDisplayUnit(), newAmount, &amount_parsed))
    {
...
void TransactionView::exportClicked()
{
    ...
    writer.addColumn(SyndicateUnits::getAmountColumnTitle(model->getOptionsModel()->getDisplayUnit()), 0, TransactionTableModel::FormattedAmountRole);
...
void TransactionView::computeSum()
{
    ...
    QString strAmount(SyndicateUnits::formatWithUnit(nDisplayUnit, amount, true, SyndicateUnits::separatorAlways));
