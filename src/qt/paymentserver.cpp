...
const char* BIP71_MIMETYPE_PAYMENT = "application/syndicate-payment";
const char* BIP71_MIMETYPE_PAYMENTACK = "application/syndicate-paymentack";
const char* BIP71_MIMETYPE_PAYMENTREQUEST = "application/syndicate-paymentrequest";
                                                                       ...
                                                                       optionsModel(0)
{
    ...
    // on Mac: sent when you click syndicate: links
    ...
    if (startLocalServer) {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(0, tr("Payment request error"),
                tr("Cannot start syndicate: click-to-pay handler"));
        } else {
...
//
// OSX-specific way of handling syndicate: URIs and
// PaymentRequest mime types
//
bool PaymentServer::eventFilter(QObject* object, QEvent* event)
{
    // clicking on syndicate: URIs creates FileOpen events on the Mac
...
void PaymentServer::initNetManager()
{
    ...
    // netManager is used to fetch paymentrequests given in syndicate: URIs
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // syndicate: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
...
bool PaymentServer::processPaymentRequest(PaymentRequestPlus& request, SendCoinsRecipient& recipient)
{
    ...
    foreach (const PAIRTYPE(CScript, CAmount) & sendingTo, sendingTos) {
        ...
        if (ExtractDestination(sendingTo.first, dest)) {
            ...
            // Insecure payments to custom syndicate addresses are not supported
