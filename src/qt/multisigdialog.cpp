...
void MultisigDialog::on_createAddressButton_clicked()
{
    ...
    CSyndicateAddress address(scriptID);
...
void MultisigDialog::on_saveMultisigAddressButton_clicked()
{
    ...
    if(!wallet->HaveCScript(scriptID))
        wallet->AddCScript(script);
    if(!wallet->mapAddressBook.count(CSyndicateAddress(address).Get()))
        wallet->SetAddressBookName(CSyndicateAddress(address).Get(), label);
}
    ...
    for(int i = 0; i < ui->outputs->count(); i++)
    {
        ...
        if(entry)
        {
            if(entry->validate())
            {
                ...
                CSyndicateAddress address(recipient.address.toStdString());
