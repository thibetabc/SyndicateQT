...
{
    // Convert walle:// to walle:
    //
    //    Cannot handle this later, because walle:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open walle.conf with the associated application */
...
{
    return GetAutostartDir() / "walle.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a walle.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the walle app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add walle app to startup item list
