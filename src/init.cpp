...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 9999)") + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22348)") + "\n";
    ...
    strUsage += "\n" + _("SSL options: (see the Syndicate Wiki for SSL setup instructions)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeionamount=<n> " + _("Keep N Syndicate anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (fDaemon) fprintf(stdout, "Syndicate server starting\n"); 
...
#ifdef ENABLE_WALLET
    if (mapArgs.count("-reservebalance")) // syndicate: reserve balance amount
    {
    ...
    // requested to kill syndicate-qt during the last operation. If so, exit.
    ...
    nAnonymizeSyndicateAmount = GetArg("-anonymizeionamount", 0);
