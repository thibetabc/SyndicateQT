...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // syndicate: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // syndicate: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // syndicate: track money supply and mint amount info
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    // syndicate: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // syndicate: compute stake entropy bit for stake modifier
    ...
    // syndicate: compute stake modifier
...
// ----------- instantX transaction scanning -----------
    ...
    BOOST_FOREACH(const CTransaction& tx, vtx)
    {
        ...
        // syndicate: check transaction timestamp
...
#ifdef ENABLE_WALLET
// syndicate: attempt to generate suitable proof-of-stake
