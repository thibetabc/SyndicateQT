                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Syndicate.conf if it does not exist
    ...
    {
        // Don't overwrite existing settings so command line settings override Syndicate.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "syndicate.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Syndicate will not work properly.");
