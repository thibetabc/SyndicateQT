...
class CTxIn;
class CStashedsendPool;
class CStashedSendSigner;
class CMasterNodeVote;
class CSyndicateAddress;
class CStashedsendQueue;
class CStashedsendBroadcastTx;
...
#define STASHEDSEND_QUEUE_TIMEOUT                 30
#define STASHEDSEND_SIGNING_TIMEOUT               15

// used for anonymous relaying of inputs/outputs/sigs
#define STASHEDSEND_RELAY_IN                 1
#define STASHEDSEND_RELAY_OUT                2
#define STASHEDSEND_RELAY_SIG                3

extern CStashedsendPool stashedSendPool;
extern CStashedSendSigner stashedSendSigner;
extern std::vector<CStashedsendQueue> vecStashedsendQueue;
extern std::string strMasterNodePrivKey;
extern map<uint256, CStashedsendBroadcastTx> mapStashedsendBroadcastTxes;
extern CActiveMasternode activeMasternode;

/** Holds an Stashedsend input
 */
...
// A clients transaction in the stashedsend pool
class CStashedSendEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for Stashedsend
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > STASHEDSEND_QUEUE_TIMEOUT;// 120 seconds
    }
};


/**
 * A currently inprogress Stashedsend merge and denomination information
 */
class CStashedsendQueue
{
public:
    ...
    /** Sign this Stashedsend transaction
    ...
    /// Is this Stashedsend expired?
    bool IsExpired()
    {
        return (GetTime() - time) > STASHEDSEND_QUEUE_TIMEOUT;// 120 seconds
    }
...
/** Helper class to store Stashedsend transaction (tx) information.
 */
class CStashedsendBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 5000 SYNX - checking if valid masternode)
...
/** Used to keep track of current status of Stashedsend pool
 */
class CStashedsendPool
{
private:
    mutable CCriticalSection cs_stashedsend;

    std::vector<CStashedSendEntry> entries; // Masternode entries
...
public:
    ...
    CStashedsendPool()
    {
        /* Stashedsend uses collateral addresses to trust parties entering the pool
    ...
    /** Process a Stashedsend message using the Stashedsend protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | Stashedsend Acceptable
     *        dsc      | Stashedsend Complete
     *        dsf      | Stashedsend Final tx
     *        dsi      | Stashedsend vIn
     *        dsq      | Stashedsend Queue
     *        dss      | Stashedsend Signal Final Tx
     *        dssu     | Stashedsend status update
     *        dssub    | Stashedsend Subscribe To
     * \param vRecv
     */
    void ProcessMessageStashedsend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress(){
        SetCollateralAddress(Params().StashedsendPoolDummyAddress());
    }
    ...
    void UpdateState(unsigned int newState)
    {
        if (fMasterNode && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("stashedsend", "CStashedsendPool::UpdateState() - Can't set state to ERROR or SUCCESS as a Masternode. \n");
            return;
        }

        LogPrintf("CStashedsendPool::UpdateState() == %d | %d \n", state, newState);
        if(state != newState){
    ...
    /// Passively run Stashedsend in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false);
    bool PrepareStashedsendDenominate();

    /// from masternode-sync.h
    bool IsBlockchainSynced();

    /// Check for process in Stashedsend
    void Check();
    ...
    void SendStashedsendDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get Masternode updates about the progress of Stashedsend
