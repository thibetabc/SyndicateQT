...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help")) 
        ...
        std::string strUsage = strprintf(_("%s syndicate-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const std::string& strInput)
{
    ...
    CSyndicateAddress addr(strAddr);
    if (!addr.IsValid())
...
*/
    if (bScriptHash) {
        ...
        CSyndicateAddress redeemScriptAddr(scriptPubKey);
